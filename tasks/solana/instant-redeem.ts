...
const buildComposeMsg = (recipient: Uint8Array) =>
    utils.arrayify(
            ...
            [
                {
                    ...
                    minAmountLD: 0,
                    extraOptions: '0x',
                    composeMsg: utils.defaultAbiCoder.encode(['uint8'], [0]),
                    oftCmd: '0x',
                },
                0,
            ],
...
task('lz:oft:nest:instant-redeem', 'Instant Redeem nest tokens cross‐chain from Solana to any supported chain')
    ...
    .setAction(async (args: MasterArgs, hre: HardhatRuntimeEnvironment) => {
        ...
        if (chainType === ChainType.EVM) {
            ...
            const composeMsgBytes = buildComposeMsg(pdaBytes)
