    ...
    PausableUpgradeable
{
    ...
    function initialize(address _owner, address _serviceManager, string memory _policyID) external initializer {
        if (_owner == address(0) || _serviceManager == address(0)) revert Errors.ZeroAddress();
    ...
    /// @notice Allows users to deposit into the NestVault using Permit2 signature-based transfer
    /// @dev    Publicly callable. Uses the predicate authorization pattern to validate the transaction
    ///         and Permit2 SignatureTransfer for gasless token approvals.
    ...
    function depositWithPermit2(
        ...
        _permit2.permitTransferFrom(
            ...
            ISignatureTransfer.PermitTransferFrom({
                permitted: ISignatureTransfer.TokenPermissions({token: address(_depositAsset), amount: _depositAmount}),
                nonce: _nonce,
                deadline: _deadline
            }),
            // The transfer recipient and amount.
            ISignatureTransfer.SignatureTransferDetails({to: address(this), requestedAmount: _depositAmount}),
            // The owner of the tokens, which must also be the signer of the message.
            msg.sender,
    ...
    function deposit(
        ...
        bytes memory _encodedSigAndArgs = abi.encodeWithSignature("deposit(bytes32)", _depositor);
        if (!_authorizeTransaction(_predicateMessage, _encodedSigAndArgs, msg.sender, 0)) {
            revert Errors.NestPredicateProxyPredicateUnauthorizedTransaction();
        }
    ...
    /// @dev    This is NOT an actual function that is called, it serves as a function to allow any contract to check a user
    ///         against the predicate
    /// @param  _user             bytes32          bytes32 representation of the user (for non-EVM addresses)
    /// @param  _predicateMessage PredicateMessage Predicate message to authorize the transaction
    /// @return                   bool             returns `true` is user is authorized to call predicate
    function genericUserCheckPredicate(bytes32 _user, PredicateMessage calldata _predicateMessage)
        external
        returns (bool)
    {
        //@dev This is NOT an actual function that is called, it is the against which the predicate is authorized
        bytes memory encodedSigAndArgs = abi.encodeWithSignature("accessCheck(bytes32)", _user);
        //still use 0 for msg.value since we only need validation against sender and user address
