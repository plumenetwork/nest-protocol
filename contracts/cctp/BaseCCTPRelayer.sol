...
contract BaseCCTPRelayer is Initializable, AuthUpgradeable, ReentrancyGuardTransientUpgradeable {
        ...
        returns (bool _relaySuccess, bool _hookSuccess)
    {
        ...
        // Relay message
        (_relaySuccess, amountReceived) = _receiveMessage(_message, _attestation);
        if (!_relaySuccess) return (false, false);
        ...
        try this.executeHook{value: msg.value}(amountReceived, _message, _data) {
    ...
    /// @param _message         bytes calldata  The encoded CCTP message.
    /// @param _data            bytes calldata  Additional data for hook execution.
    function executeHook(uint256 _amountReceived, bytes calldata _message, bytes calldata _data)
        public
        payable
        virtual
    {
        ...
        (bool success,) = _executeHook(_amountReceived, _message, _data);
    ...
    function getHookData(bytes calldata _message, bytes calldata _data) public view returns (bytes29 _hookData) {
        _hookData = _getHookData(_message, _data);
    }
    ...
    function validateHookData(bytes calldata _message, bytes calldata _data, uint256 _amountReceived)
        public
        view
        returns (bool _invalid, bytes memory _errorData)
    {
        bytes29 _hookData = _getHookData(_message, _data);
    ...
    /// @param _message        bytes calldata   The encoded CCTP message.
    /// @param _attestation    bytes calldata   Circle attestation proving message validity.
    /// @return _relaySuccess  bool             Whether the message was successfully relayed.
    ...
    function _receiveMessage(bytes calldata _message, bytes calldata _attestation)
        internal
        returns (bool _relaySuccess, uint256 _amountReceived)
    {
        // Validate message
        ...
        _relaySuccess = MESSAGE_TRANSMITTER.receiveMessage(_message, _attestation);

        if (!_relaySuccess) revert Errors.MessageRelayFailed();
    ...
    function _executeHook(uint256 _amountReceived, bytes calldata _message, bytes calldata _data)
        internal
        virtual
        returns (bool _success, bytes memory _returnData)
    {
        bytes29 hookData = _getHookData(_message, _data);
