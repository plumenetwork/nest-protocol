...
contract NestCCTPRelayer is BaseCCTPRelayer, IOFT {
    ...
    function quoteRelay(bytes calldata _message, bytes calldata _data)
        external
        view
        returns (MessagingFee memory _quote)
    {
        ...
        (address target,,, uint256 amount, SendParam memory sendParam,) =
            _decodeNestHookData(_getHookData(_message, _data));
    ...
    function _getHookData(bytes memory _message, bytes memory _data)
        internal
        view
        override
