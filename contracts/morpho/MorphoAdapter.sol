...
import {IWNative} from "contracts/vendor/bundler3/interfaces/IWNative.sol";
import {MarketParams, IMorpho} from "@morpho/interfaces/IMorpho.sol";
import {CoreAdapter, ErrorsLib, IERC20, SafeERC20, Address} from "contracts/vendor/bundler3/adapters/CoreAdapter.sol";
import {MathRayLib} from "contracts/vendor/bundler3/libraries/MathRayLib.sol";
import {SafeCast160} from "@uniswap/permit2/libraries/SafeCast160.sol";
import {Permit2Lib} from "@uniswap/permit2/libraries/Permit2Lib.sol";
...
contract MorphoAdapter is CoreAdapter {
    ...
    /// @notice Withdraws collateral on Morpho for a specific owner.
    /// @dev `onBehalf` must have authorized both the adapter on Morpho (enforced by Morpho) and the current
    ///      Bundler initiator (enforced here).
    /// @param marketParams Morpho market to withdraw collateral from.
    /// @param assets Collateral amount to withdraw. Pass `type(uint256).max` to withdraw all owner collateral.
    /// @param onBehalf Owner of the collateral position being withdrawn.
    /// @param receiver Receiver of collateral assets.
    function morphoWithdrawCollateralOnBehalf(
        MarketParams calldata marketParams,
        uint256 assets,
        address onBehalf,
        address receiver
        ...
        if (onBehalf != _initiator) {
            require(MORPHO.isAuthorized(onBehalf, _initiator), ErrorsLib.UnauthorizedSender());
        }

        if (assets == type(uint256).max) assets = MorphoLib.collateral(MORPHO, marketParams.id(), onBehalf);
        require(assets != 0, ErrorsLib.ZeroAmount());

        MORPHO.withdrawCollateral(marketParams, assets, onBehalf, receiver);
    }

    /// @notice Borrows on Morpho for a specified position owner.
    /// @dev Callable only from Bundler3.
    ///      If `onBehalf` is different from the current initiator, the owner must authorize the initiator on Morpho.
    /// @param marketParams Morpho market to borrow from.
    /// @param assets Amount of assets to borrow.
    /// @param shares Amount of borrow shares to mint (mutually exclusive with `assets`).
    /// @param minSharePriceE27 Minimum assets per borrow share, scaled by 1e27.
    /// @param onBehalf Owner of the debt position.
    /// @param receiver Recipient of borrowed assets.
    function morphoBorrowOnBehalf(
        MarketParams calldata marketParams,
        uint256 assets,
        uint256 shares,
        uint256 minSharePriceE27,
