...
/// @title DeployNestShareOFT_nALPHA_USDC
/// @notice Deployment script for nALPHA share NestShareOFT with USDC as underlying asset
/// @dev This script deploys NestShareOFT for the nALPHA share with USDC asset.
///      Share and asset configurations are read from L0Config.json.
///      Uses the shared ProxyAdmin from L0Config to manage the proxy.
///      To run: forge script script/DeployNestOFTProtocol/DeployNestShareOFT_nALPHA_USDC.s.sol --rpc-url $RPC_URL --broadcast
contract DeployNestShareOFT_nALPHA_USDC is DeployNestShareOFT {
    ...
    function setupEvms() public override {
        ...
        nestVaultOfts[0] = expectedNestVaultOfts[0]; // index 0 for nALPHA
    ...
    /// @dev Override to only deploy NestShareOFT for nALPHA share with USDC asset
    function deployOFTs() public override broadcastAs(oftDeployerPK) {
        ...
        // Find nALPHA share
        NestShareConfig memory nAlphaShareConfig;
        bool foundShare = false;
        for (uint256 v = 0; v < broadcastConfig.nestShareConfigs.length; v++) {
            if (isStringEqual(broadcastConfig.nestShareConfigs[v].symbol, "nALPHA")) {
                nAlphaShareConfig = broadcastConfig.nestShareConfigs[v];
                foundShare = true;
                break;
            }
        }
        require(foundShare, "nALPHA share not found in config");
    ...
    function postDeployChecks() internal view override {
        require(nestShareOFTs.length == 1, "Should deploy exactly 1 NestShareOFT (nALPHA)");
        require(nestShareOFTs[0] != address(0), "Invalid nALPHA OFT proxy address");
    }
    ...
    function _validateAndPopulateMainnetOfts() internal virtual override {
        ...
        connectedOfts[0] = expectedNestVaultOfts[0]; //index 0 is nALPHAVaultOFT_USDC
    }
