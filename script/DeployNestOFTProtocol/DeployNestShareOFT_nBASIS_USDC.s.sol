...
/// @title DeployNestShareOFT_nBASIS_USDC
/// @notice Deployment script for nBASIS share NestShareOFT with USDC as underlying asset
/// @dev This script deploys NestShareOFT for the nBASIS share with USDC asset.
///      Share and asset configurations are read from L0Config.json.
///      Uses the shared ProxyAdmin from L0Config to manage the proxy.
///      To run: forge script script/DeployNestOFTProtocol/DeployNestShareOFT_nBASIS_USDC.s.sol --rpc-url $RPC_URL --broadcast
contract DeployNestShareOFT_nBASIS_USDC is DeployNestShareOFT {
    ...
    function setupEvms() public override {
        ...
        nestVaultOfts[0] = expectedNestVaultOfts[1]; // index 1 for nBASIS
    ...
    /// @dev Override to only deploy NestShareOFT for nBASIS share with USDC asset
    function deployOFTs() public override broadcastAs(oftDeployerPK) {
        ...
        // Find nBASIS share
        NestShareConfig memory nBasisShareConfig;
        bool foundShare = false;
        for (uint256 v = 0; v < broadcastConfig.nestShareConfigs.length; v++) {
            if (isStringEqual(broadcastConfig.nestShareConfigs[v].symbol, "nBASIS")) {
                nBasisShareConfig = broadcastConfig.nestShareConfigs[v];
                foundShare = true;
                break;
            }
        }
        require(foundShare, "nBASIS share not found in config");
    ...
    function postDeployChecks() internal view override {
        require(nestShareOFTs.length == 1, "Should deploy exactly 1 NestShareOFT (nBASIS)");
        require(nestShareOFTs[0] != address(0), "Invalid nBASIS OFT proxy address");
    }
    ...
    function _validateAndPopulateMainnetOfts() internal virtual override {
        ...
        connectedOfts[0] = expectedNestVaultOfts[1]; //index 1 is nBASISVaultOFT_USDC
    }
