...
abstract contract Denylistable is Ownable2Step {
    ...
    modifier onlyDenylister() {
        require(msg.sender == _denylister, "Denylistable: caller is not denylister");
        _;
    }
    ...
    function _updateDenylister(address _newDenylister) internal {
        require(_newDenylister != address(0), "Denylistable: new denylister is the zero address");
    ...
    function _requireNotDenylisted(address _address) internal view {
        require(_denylist[_address] == _FALSE, "Denylistable: account is on denylist");
    }
}
