...
abstract contract Attestable is Ownable2Step {
    // ============ Events ============
    /**
    ...
    function disableAttester(address attester) external onlyAttesterManager {
        ...
        require(_numEnabledAttesters > signatureThreshold, "Signature threshold is too low");
    ...
    function _setAttesterManager(address _newAttesterManager) internal {
        require(_newAttesterManager != address(0), "Invalid attester manager address");
    ...
    function _verifyAttestationSignatures(bytes calldata _message, bytes calldata _attestation) internal view {
        require(_attestation.length == signatureLength * signatureThreshold, "Invalid attestation length");
        ...
        for (uint256 i; i < signatureThreshold; ++i) {
            bytes memory _signature = _attestation[i * signatureLength:i * signatureLength + signatureLength];

            address _recoveredAttester = _recoverAttesterSignature(_digest, _signature);

            // Signatures must be in increasing order of address, and may not duplicate signatures from same address
            require(_recoveredAttester > _latestAttesterAddress, "Invalid signature order or dupe");
            require(isEnabledAttester(_recoveredAttester), "Invalid signature: not attester");
     ...
     *
     */
    function _recoverAttesterSignature(bytes32 _digest, bytes memory _signature) internal pure returns (address) {
    ...
    function _setSignatureThreshold(uint256 _newSignatureThreshold) internal {
        ...
        require(_newSignatureThreshold <= enabledAttesters.length(), "New signature threshold too high");

        require(_newSignatureThreshold != signatureThreshold, "Signature threshold already set");
