...
contract TokenMessengerV2 is IMessageHandlerV2, BaseTokenMessenger {
    ...
    function initialize(
        ...
        require(remoteDomains_.length == remoteTokenMessengers_.length, "Invalid remote domain configuration");
        ...
        for (uint256 i; i < _remoteDomainsLength; ++i) {
            _addRemoteTokenMessenger(remoteDomains_[i], remoteTokenMessengers_[i]);
        }
    }
     ...
     */
    function handleReceiveFinalizedMessage(uint32 remoteDomain, bytes32 sender, uint32, bytes calldata messageBody)
        external
        override
    ...
    ) external override onlyLocalMessageTransmitter onlyRemoteTokenMessenger(remoteDomain, sender) returns (bool) {
        require(finalityThresholdExecuted >= TOKEN_MESSENGER_MIN_FINALITY_THRESHOLD, "Unsupported finality threshold");
    ...
    function _calcMinFeeAmount(uint256 _amount) internal view returns (uint256) {
        uint256 _minFeeAmount = _amount * minFee / MIN_FEE_MULTIPLIER;
    ...
    function _depositForBurn(
        ...
        // Send message
        IRelayerV2(localMessageTransmitter)
            .sendMessage(
                _destinationDomain, _destinationTokenMessenger, _destinationCaller, _minFinalityThreshold, _burnMessage
            );
     ...
     * @return success Bool, true if successful.
     */
    function _handleReceiveMessage(bytes29 _msg, uint32 _remoteDomain) internal returns (bool) {
        console2.log("Handling received message from remote domain:", uint256(_remoteDomain));
        // Validate message and unpack fields
        (address _mintRecipient, bytes32 _burnToken, uint256 _amount, uint256 _fee) = _validatedReceivedMessage(_msg);
        console2.log("Mint recipient:", _mintRecipient);
        console2.log("Burn token:", AddressUtils.toAddress(_burnToken));
        console2.log("Amount:", _amount);
        console2.log("Fee:", _fee);
     ...
     */
    function _validatedReceivedMessage(bytes29 _msg)
        internal
        view
        returns (address _mintRecipient, bytes32 _burnToken, uint256 _amount, uint256 _fee)
    {
        _msg._validateBurnMessageFormat();
        require(_msg._getVersion() == messageBodyVersion, "Invalid message body version");
