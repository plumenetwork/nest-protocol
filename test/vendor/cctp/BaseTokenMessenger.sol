...
abstract contract BaseTokenMessenger is Rescuable, Denylistable, Initializable {
    ...
    event MintAndWithdraw(
        address indexed mintRecipient, uint256 amount, address indexed mintToken, uint256 feeCollected
    );
    ...
    modifier onlyMinFeeController() {
        require(msg.sender == minFeeController, "Caller is not the min fee controller");
        _;
    }
    ...
    function _depositAndBurn(address _burnToken, address _from, uint256 _amount) internal {
        ...
        require(_mintBurnToken.transferFrom(_from, address(_localMinter), _amount), "Transfer operation failed");
    ...
    function _mintAndWithdraw(
        ...
        address _mintToken;
        if (_fee > 0) {
            _mintToken = _minter.mint(_remoteDomain, _burnToken, _mintRecipient, feeRecipient, _amount, _fee);
        } else {
            _mintToken = _minter.mint(_remoteDomain, _burnToken, _mintRecipient, _amount);
        }
     ...
     */
    function _addRemoteTokenMessenger(uint32 _domain, bytes32 _tokenMessenger) internal {
        require(_tokenMessenger != bytes32(0), "bytes32(0) not allowed");

        require(remoteTokenMessengers[_domain] == bytes32(0), "TokenMessenger already set");
        console2.log("Adding remote TokenMessenger for domain:", uint256(_domain));
        console2.logBytes32(_tokenMessenger);
